...
import ma.itroad.ram.tat.scheduler.ms.service.dto.changer.DelayDto;
import ma.itroad.ram.tat.scheduler.ms.service.dto.changer.InstantMessage;
import ma.itroad.ram.tat.scheduler.ms.service.dto.changer.InstantTransactionDto;
import ma.itroad.ram.tat.scheduler.ms.service.dto.changer.LegDetail;
...
//    @Mapping(target="registration",source = "message",qualifiedByName = "registration")
    ...
    default String mapDelayCode(InstantMessage msg){
        try{
            ...
            for(DelayDto delay : delays){
                if(!delay.equals("")) s=s+delay.getDelayCode()+";";
            }
            return s;
        }
        catch(Exception e){
    ...
    default String mapDelayTime (InstantMessage msg){
        try{
            ...
            List<DelayDto> delays = msg.getLegDetail().getActuals().getDelay();
            for(DelayDto delay : delays){
                if(!delay.equals("")) s =s+delay.getDelayTime()+";";
            }
            return s;
        }
        catch(Exception e){
    ...
    default String mapAsmActionIdentifier(InstantMessage msg){
        try{
            ...
            for(String asmAction : asmActions){
                if(!asmAction.equals("")) s =s+asmAction+";";
            }
            return s;
        }
        catch (Exception e )
